...
If you have an SSD (and it's 2025, you either have an SSD now or you won't have
anything later), you should also pass the `noatime` option to save on
unnecessary writes. By default, the Linux kernel updates file access times
_every single time_ a file is opened, which is rather inefficient and isn't even
all that useful.

Turning access times off is less important for those who are still using HDDs,
but it still can't hurt.
</Callout>
