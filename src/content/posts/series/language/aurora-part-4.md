...
Let's add `Sources/Aurora/Evaluation/Value.swift`:
...
1. Recursively call `evaluate()` on `lhs` and `rhs`, and extract `number`
   values from the results. Since `Value` only has the one case right now, this
...
we should probably evaluate those expressions and output the result of that.
But I would like to keep the existing behavior around for debugging purposes,
so we'll go ahead and add a command-line flag.
...
Actually, let's factor out the contents of that `do` block so we don't end up
in indentation hell later on:
