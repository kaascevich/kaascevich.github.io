...
> **UPDATE: #1** I've found a (temporary) solution to the first problem
...
> 3. An alert will appear asking if you really want to kill `coreaudiod`.
>    Choose Quit, and authenticate if asked. `coreaudiod` will immediately
>    restart after you kill it; this is expected.
> 4. AXVisualSupportAgent should immediately stop using the microphone, and
>    won't use it again until your Mac restarts.
>
> Alternatively, if you're of the do-it-quickly type, just run this in a
> terminal:
>
> ```console
...
> ```
>
> You will need to perform these steps again every time you restart your Mac
> until Apple provides an official fix.
...
`/System/Library/PrivateFrameworks/UniversalAccess.framework/Versions/A/Resources/AXVisualSupportAgent.app`.[^1]
It seems to be responsible for handling the screen zoom feature of macOS. As
far as I'm aware, this thing should have _no reason_ to be using the mic.

Yet, checking the `entitlements.plist` embedded within the executable shows
that AXVisualSupportAgent does indeed have permission to use the mic without
user authorization:
...
accessibility features _enabled_, let alone being used. So _why is this thing
constantly using my microphone_?
...
[^1]: The fact that this app is located in the System folder is enough to prove
  ...
  System volume has been modified in _any_ way. You can read more about this
  [here][ssv].
